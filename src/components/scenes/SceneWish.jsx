...
export default function SceneWish({ data, onNext }) {
  return (
    <Box sx={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden', bgcolor: '#f7eeee', background: 'radial-gradient(ellipse at 50% 45%, rgba(255,255,255,.7), transparent 68%), repeating-linear-gradient(0deg, rgba(205,153,145,.035) 0 1px, transparent 1px 5px), #f7eeee', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', px: { xs: 2.5, sm: 4 }, py: 2 }}>
      ...
      <Typography variant="h1" sx={{ zIndex: 1, color: '#c84f3d', fontFamily: 'Caveat, cursive', fontWeight: 700, fontSize: { xs: 34, sm: 45 }, lineHeight: 1, textAlign: 'center', transform: 'rotate(-2deg)', mb: 1.5, animation: 'wish-title-in .6s ease both', '@keyframes wish-title-in': { from: { opacity: 0, transform: 'translateY(-12px) rotate(-5deg)' }, to: { opacity: 1, transform: 'translateY(0) rotate(-2deg)' } } }}>MY WISH FOR U</Typography>

      <Box sx={{ zIndex: 1, width: 'min(100%, 445px)', maxHeight: '64%', overflow: 'hidden', px: { xs: 1, sm: 2 }, py: 1, borderTop: '1px solid rgba(196,92,81,.12)', borderBottom: '1px solid rgba(196,92,81,.12)', animation: 'wish-copy-in .7s ease .2s both', '@keyframes wish-copy-in': { from: { opacity: 0, transform: 'translateY(8px)' }, to: { opacity: 1, transform: 'translateY(0)' } } }}>
        <Typography component="p" sx={{ m: 0, color: '#403b39', fontFamily: 'Patrick Hand, cursive', fontSize: { xs: 12.5, sm: 15 }, fontWeight: 400, lineHeight: 1.32, textAlign: 'center' }}>{data.wish}</Typography>
      </Box>
