  ...
  return (
    <Box sx={{ position: 'relative', width: '100%', height: '100%', overflow: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', bgcolor: '#f7eeee', background: 'radial-gradient(ellipse at 50% 45%, rgba(255,255,255,.7), transparent 68%), #f7eeee', px: 2 }}>
      ...
      <Box aria-label="Photo collage of the couple" sx={{ position: 'relative', width: 'min(92%, 380px)', height: { xs: 260, sm: 310 }, mt: 1, animation: 'proposal-collage-in .8s ease .15s both', '@keyframes proposal-collage-in': { from: { opacity: 0, transform: 'scale(.9)' }, to: { opacity: 1, transform: 'scale(1)' } } }}>
        ...
        <Photo src="/girl.jpeg" alt="Her portrait" sx={{ position: 'absolute', zIndex: 2, width: { xs: 150, sm: 190 }, height: { xs: 190, sm: 235 }, left: '50%', top: 22, transform: 'translateX(-50%)', clipPath: 'polygon(50% 100%, 8% 60%, 0 28%, 17% 6%, 38% 13%, 50% 0, 62% 13%, 83% 6%, 100% 28%, 92% 60%)' }} />
        <Photo src="/boy.jpeg" alt="His portrait" sx={{ position: 'absolute', zIndex: 3, width: { xs: 92, sm: 112 }, height: { xs: 92, sm: 112 }, left: '7%', bottom: 18, borderRadius: '50%', border: '4px solid #f7eeee', transform: 'rotate(-12deg)' }} />
        <Photo src="/girl.jpeg" alt="Her portrait" sx={{ position: 'absolute', zIndex: 3, width: { xs: 86, sm: 105 }, height: { xs: 86, sm: 105 }, right: '6%', bottom: 27, borderRadius: '50%', border: '4px solid #f7eeee', transform: 'rotate(12deg)' }} />
